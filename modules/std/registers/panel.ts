...
const registry = new (class extends Registry<React.ReactNode> {
  private static NodeHash = Symbol.for("NodeHash");
  ...
  getHash(value: React.ReactNode) {
    ...
    // @ts-expect-error
    const hash = value[this.constructor.NodeHash];
  ...
  override add(value: React.ReactNode, onEntry?: any, onExit?: any): this {
    ...
    // @ts-expect-error
    value[this.constructor.NodeHash] = hash;
    const state = `bespoke_${hash}`;
    const event = `bespoke_${hash}_button_click`;
  ...
  override delete(item: React.ReactNode): boolean {
    // @ts-expect-error
    const hash = item[this.constructor.NodeHash];
    const state = `bespoke_${hash}`;
    const event = `bespoke_${hash}_button_click`;
...
registerRegistry("panel", registry);
    ...
    Object.defineProperty(globalThis, "__Machine", {
      set: ($: StateMachine) => {
        ...
        queueMicrotask(() => {
          ...
          for (const [_k, v] of Object.entries(Machine.config.states!)) {
            v.on = new Proxy(v.on!, {
